...
public class GUIFavourites extends Gui {
    ...
    private void draw() {
        ...
        for (Skull skull : data) {
            setItem(slot, SkullAPI.getInstance().getTexturedHead(skull.getBase64(), true, Settings.GUI_FAVOURITES_HEAD_TITLE.getString(), Settings.GUI_FAVOURITES_HEAD_LORE.getStringList(), new HashMap<String, Object>(){{
            ...
            setAction(slot, ClickType.SHIFT_RIGHT, e -> {
                if (!e.player.hasPermission("skulls.editprice")) {
                    ...
                    }).setOnCancel(() -> e.manager.showGUI(e.player, new GUIFavourites())).setOnClose(() -> e.manager.showGUI(e.player, new GUIFavourites()));
                }
            });
